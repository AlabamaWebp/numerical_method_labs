...
function buildFiniteDifferenceTable(y) {
    const n = y.length;
    const table = [y.slice()];
...
function printFiniteDifferenceTable(xNodes, table) {
...
function newtonForwardInterpolation(x, x0, h, table) {
    const q = (x - x0) / h;
...
function newtonBackwardInterpolation(x, xNodes, h, table) {
    const n = xNodes.length;
    const xn = xNodes[n - 1];
    const q = (x - xn) / h;
...
function gaussCentralInterpolation(x, xNodes, h, table) {
    const n = xNodes.length;
